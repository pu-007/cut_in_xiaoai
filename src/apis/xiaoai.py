#!/usr/bin/python
# -*- coding: UTF-8 -*-
# https://developers.xiaoai.mi.com/documents/Home?type=/api/doc/render_markdown/SkillAccess/skill/fulu/py
import json
        ...
        if not kwargs:
            ...
            for name, field_type, is_required in self._fields:
                if is_required and name not in kwargs.keys():
                    raise TypeError(
                        "Key: {} is required but not found in the json data".format(
                            name
                        )
                    )
                v = kwargs.pop(name, {})
...
class XiaoAIUser(XiaoAIBase):
    _fields = [
        ...
        ("is_user_login", bool, False),
        ("gender", str, False),
        ("real_user_id", str, False),
    ]
...
class XiaoAISession(XiaoAIBase):
    _fields = [
        ("is_new", bool, True),
        ("session_id", str, True),
        ("application", XiaoAIApplication, True),
        ("attributes", dict, False),
        ("user", XiaoAIUser, False),
    ]
...
class XiaoAIRequest(XiaoAIBase):
    _fields = [
        ...
        ("timestamp", int, True),
        ("locale", str, True),
        ("intent", XiaoAIIntent, False),
        ("no_response", bool, False),
        ("type", int, True),
        ("slot_info", XiaoAISlotInfo, False),
...
class XiaoAIOpenRequest(XiaoAIBase):
    _fields = [
        ("version", str, True),
        ("query", str, False),
        ("session", XiaoAISession, True),
        ("context", XiaoAIContext, False),
        ("request", XiaoAIRequest, True),
    ]
...
class XiaoAIUITemplate:
    ...
    def __init__(
        self, type_, items=None, item=None, logo=None, package_name=None, logo_text=None
    ):
        if type_ is not None and type(type_) is not int:
...
class XiaoAIToDisplay:
    ...
    def __init__(
        ...
        if (
            phone_template is not None
            and type(phone_template) is not XiaoAIPhoneTemplate
        ):
...
class XiaoAIStream:
    ...
    def __init__(self, url, token=None, offset_in_milliseconds=None):
        ...
        self.token = token
        if (
            offset_in_milliseconds is not None
            and type(offset_in_milliseconds) is not int
        ):
...
class XiaoAIActionProperty:
    ...
    def __init__(
        ...
        quick_app_path=None,
        push_app_info=None,
        quick_app_extra=None,
        device_command=None,
        app_intent_info=None,
        quick_app_card_info=None,
        app_h5_url=None,
    ):
        ...
        if (
            push_app_info is not None
            and type(push_app_info) is not XiaoAIPushAppActionProperty
        ):
            raise TypeError(
        ...
        if (
            ...
            and type(device_command) is not XiaoAIDeviceCommandActionProperty
        ):
            raise TypeError(
        ...
        if (
            app_intent_info is not None
            and type(app_intent_info) is not XiaoAIIntentInfoExtra
        ):
            raise TypeError(
        ...
        if (
            quick_app_card_info is not None
            and type(quick_app_card_info) is not XiaoAIQuickAppCardActionProperty
        ):
            raise TypeError(
...
class XiaoAIResponse:
    ...
    def __init__(
        ...
        if (
            action_property is not None
            and type(action_property) is not XiaoAIActionProperty
        ):
        ...
        if (
            is_directive_not_interrupted is not None
            and type(is_directive_not_interrupted) is not bool
        ):
...
def xiaoai_response(data):
    return json.dumps(
        data,
        default=lambda obj: dict(
            ("type" if key == "type_" else key, value)
